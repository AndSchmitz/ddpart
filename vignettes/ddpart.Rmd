...
there are 4 variants of parametrization for the Zhang01 approach:
...
## 2. Dry deposition velocity vs. particle size
...
Note that Emerson et al. state that their Fig. 1 has been generated with a
friction velocity of 0.2 m/s. Three observations strongly suggest that this is
not the case and that instead the same friction velocity as in Emerson et al.
Fig. 2 (0.4 m/s) has been used by Emerson to generate Fig. 1. These observations
are:
...
below based on the data extracted from Emerson et al.

- Emerson Fig. 2 has been generated with friction velocity of 0.4 m/s according
to its figure caption.
...
TableData <- Output %>%
  ...
  mutate(
    LUCShort = case_when(
      LUCNames == "Needleleaf" ~ "Needle",
      LUCNames == "DecBroadleaf" ~ "DecBL",
      LUCNames == "Grassland" ~ "Grass",
      T ~ "ERROR"
    ),
...
This graph shows that the relation between dry deposition velocity and particle
size are exactly identical in Emerson Fig. 1 and Emerson Fig. 2. This strongly
suggests that Emerson used the same value of friction velocity to generate these
graphs.
 ...
 Emerson Fig 1. Here, only the data for LUC1 (needleleaf forest) is used.
...
EmersonCmp <- bind_rows(EmersonFig1, EmersonFig2) %>%
  mutate(
    Parametrization = factor(
      ...
      levels = c("Parametrization = GEOS-Chem old", "Parametrization = GEOS-Chem new")
    )
  )
