...
InputTable_UnstableConditions <- data.frame(
  ...
  WindSpeed_AtAnemometerHeight_ms = 3,
  ...
  ZeroPlaneDisplacementHeightTargetLUC = 0.21,
  #Particle properties
  GeometricMassMedianDiameter_m =  4.5 * 1e-6,
...
InputTable_StableConditions <- InputTable_UnstableConditions %>%
  mutate(
    ...
    WindSpeed_AtAnemometerHeight_ms = 1,
...
TableDat <- Results %>%
  select(Label, RoughnessLengthAnemometer_m, WindSpeed_AtAnemometerHeight_ms, PasquillClass, ObukhovLength_Anemometer_m) %>%
  distinct()
kable(
  x = TableDat
)
...
Results <- Results %>%
  mutate(
    ...
    WindSpeedAtBlendingHeight_ms_PowerLaw = WindSpeed_AtAnemometerHeight_ms * (WindSpeedBlendingHeight_m / AnemometerHeight_m)^PowerLawExponent
  ) %>%
  rename(
...
InputTable <- data.frame(
  ...
  WindSpeed_AtAnemometerHeight_ms = 7.8,
